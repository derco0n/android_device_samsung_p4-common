...
static int legacy_out_write(struct audio_stream_out *stream, const void* buffer,
                         size_t bytes)
{
    int ret = 0;
    struct stream_out *out = (struct stream_out *)stream;
                     ...
                     hw_device_t** device)
{
    ...
    *device = &adev->hw_device.common;
    ...
    err = uname(&kernel_info);
    if (!err) {
        if (!strncmp(kernel_info.release, "3.1.10", 6))
            adev->legacy_kernel = true;
        else
            adev->legacy_kernel = false;

        ALOGI("%s() uname=%s", __func__, kernel_info.release);
        ALOGI("%s() legacy_kernel=%s", __func__,
            adev->legacy_kernel ? "true" : "false");
    } else {
        ALOGE("%s() uname error: %s", __func__, strerror(errno));
    }
